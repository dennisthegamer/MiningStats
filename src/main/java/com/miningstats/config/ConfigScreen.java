...
public class ConfigScreen {
    ...
    public static Screen create(Screen parent) {
        ...
        var hudCategory = ConfigCategory.createBuilder()
                .name(Text.translatable("config.miningstats.category.hud"))
                .option(Option.<ModConfig.HudPosition>createBuilder()
                        .name(Text.translatable("config.miningstats.hud_position"))
                        .description(val -> OptionDescription.of(
                                Text.translatable("config.miningstats.hud_position.tooltip")))
                ...
                .option(Option.<Boolean>createBuilder()
                        .name(Text.translatable("config.miningstats.hud_visible_always"))
                        .description(val -> OptionDescription.of(
                                Text.translatable("config.miningstats.hud_visible_always.tooltip")))
                        .binding(defaults.hudVisibleAlways, () -> config.hudVisibleAlways, v -> config.hudVisibleAlways = v)
                        .controller(TickBoxControllerBuilder::create)
                        .build())
                .option(Option.<Float>createBuilder()
                        .name(Text.translatable("config.miningstats.hud_opacity"))
                        .description(val -> OptionDescription.of(
                                Text.translatable("config.miningstats.hud_opacity.tooltip")))
        ...
        var sessionCategory = ConfigCategory.createBuilder()
                .name(Text.translatable("config.miningstats.category.session"))
                .option(Option.<Boolean>createBuilder()
                        .name(Text.translatable("config.miningstats.show_session_summary"))
                        .description(val -> OptionDescription.of(
                                Text.translatable("config.miningstats.show_session_summary.tooltip")))
                        .binding(defaults.showSessionSummary, () -> config.showSessionSummary, v -> config.showSessionSummary = v)
                        .controller(TickBoxControllerBuilder::create)
                        .build())
                .option(Option.<Boolean>createBuilder()
                        .name(Text.translatable("config.miningstats.persist_sessions"))
                        .description(val -> OptionDescription.of(
                                Text.translatable("config.miningstats.persist_sessions.tooltip")))
        ...
        var trackingCategory = ConfigCategory.createBuilder()
                .name(Text.translatable("config.miningstats.category.tracking"))
                .option(Option.<Boolean>createBuilder()
                        .name(Text.translatable("config.miningstats.merge_deepslate"))
                        .description(val -> OptionDescription.of(
                                Text.translatable("config.miningstats.merge_deepslate.tooltip")))
                        .binding(defaults.mergeDeepslate, () -> config.mergeDeepslate, v -> config.mergeDeepslate = v)
                        .controller(TickBoxControllerBuilder::create)
                        .build())
                .group(ListOption.<String>createBuilder()
                        .name(Text.translatable("config.miningstats.tracked_ores"))
                        .description(OptionDescription.of(
                                Text.translatable("config.miningstats.tracked_ores.tooltip")))
        ...
        var milestoneCategoryBuilder = ConfigCategory.createBuilder()
                .name(Text.translatable("config.miningstats.category.milestones"));

        for (OreType type : OreType.values()) {
            if (type.isDeepslate()) continue;
            String key = type.name().toLowerCase();
            milestoneCategoryBuilder.option(Option.<Integer>createBuilder()
                    .name(Text.translatable("config.miningstats.milestone." + key))
                    .description(val -> OptionDescription.of(
                            Text.translatable("config.miningstats.milestone.tooltip", type.getDisplayName())))
                    .binding(
        ...
        return YetAnotherConfigLib.createBuilder()
                .title(Text.translatable("config.miningstats.title"))
                .save(config::save)
